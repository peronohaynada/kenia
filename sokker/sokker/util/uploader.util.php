...
class UploaderUtil {
	...
	public static function uploadFile() {
		...
		case UPLOAD_ERR_OK : // No hay error, archivo subido con �xito.
		                    // primero hay que validar tama�o del archivo!
			if ($_FILES ["userfile"] ['size'] > 0 && file_size) {
				echo $_FILES ["userfile"] ['type'];
				/*
				 * if ($_FILES["userfile"]['type'] == "application/x-tar") { UploaderUtil::untarFile(); } else
				 */
		...
		case UPLOAD_ERR_FORM_SIZE : // El archivo subido excede la directiva MAX_FILE_SIZE que fue especificada en el formulario HTML.
			break;
		case UPLOAD_ERR_PARTIAL : // El archivo subido fue s�lo parcialmente cargado.
			break;
		case UPLOAD_ERR_NO_FILE : // Ning�n archivo fue subido.
			break;
		...
		case UPLOAD_ERR_CANT_WRITE : // No se pudo escribir el archivo en el disco.
			break;
		case UPLOAD_ERR_EXTENSION : // Una extensi�n de PHP detuvo la carga de archivos.
			break;
		}
	}
	...
	public static function unzipFile() {
		/*
		 * $zip = new ZipArchive; if ($zip->open($_FILES["userfile"]['tmp_name']) === TRUE) { $zip->extractTo('./'); $zip->close(); return true; } else { return false; }
		 */
